...
function Collection(opts, getConnection) {
  ...
  this._getCollection = function _getCollection(cb) {
...
Collection.prototype.findOne = function findOne(query, projection, cb) {
...
Collection.prototype.asyncFindOne = function asyncFind(query, projections) {
  return new Promise((resolve, reject) => {
    this.findOne(query, projections, (err, doc) => {
      if (err) return reject(err);
      return resolve(doc);
    });
  });
...
Collection.prototype.asyncRemoveAll = function asyncRemoveAll() {
  return new Promise((resolve, reject) => {
    this.remove({}, { justOne: false }, (err, docs) => {
      if (err) return reject(err);
      return resolve(docs);
    });
  });
...
Collection.prototype.drop = function drop(cb) {
...
Collection.prototype.aggregate = function aggregate() {
  ...
  const pipeline = Array.prototype.slice.call(arguments);
